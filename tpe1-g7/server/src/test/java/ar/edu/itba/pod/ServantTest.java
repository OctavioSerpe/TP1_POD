...
public class ServantTest {
    ...
    /*
     * El test busca testear que al cerrar una pista de una categoria menor, reordenar y despegar, la cantidad de vuelos
     * en la pista cerrada sean 0 y la sumatoria de los vuelos que despegaron en las pistas sea igual al total de vuelos
     * agregados menos los que se perdieron al reordenar los de la pista previamente cerrada
     */
    @Test
    public void testRearrangeWithNotAllFlights() throws RemoteException, InterruptedException, ExecutionException, TimeoutException {
        final int TOTAL_TEST_FLIGHTS = 2000;
        final int TOTAL_TEST_RUNWAYS = 4;
        final int NOT_REARRANGED_FLIGHTS = 500;
        ...
        // cierro la pista de menor categoria
        servant.closeRunway(runwayNames.get(TOTAL_TEST_RUNWAYS - 1));
        final ReassignmentLog reassignmentLog = servant.rearrangeDepartures();
        Assert.assertEquals(TOTAL_TEST_FLIGHTS - NOT_REARRANGED_FLIGHTS, reassignmentLog.getAssignedCount());
        Assert.assertEquals(NOT_REARRANGED_FLIGHTS, reassignmentLog.getFailed().size());

        final ExecutorService auxExecutor = executorServiceSupplier.get();
        callables.clear();
        futures.clear();
        IntStream.range(0, TOTAL_TEST_FLIGHTS).forEach(n -> {
            callables.add(() -> {
        ...
        Assert.assertEquals(TOTAL_TEST_FLIGHTS - NOT_REARRANGED_FLIGHTS, servant.getAllDepartures().size());
        int departedFlights = 0;
        for (int i = 0; i < (TOTAL_TEST_RUNWAYS - 1); ++i) {
            departedFlights += servant.getRunwayDepartures(runwayNames.get(i)).size();
        }

        Assert.assertEquals(TOTAL_TEST_FLIGHTS - NOT_REARRANGED_FLIGHTS, departedFlights);
        Assert.assertEquals(0, servant.getRunwayDepartures(runwayNames.get(TOTAL_TEST_RUNWAYS - 1)).size());
    }
}
